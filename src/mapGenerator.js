TWA.mapGenerator = function() {
	...
	if ( /con/.test( game_data.mode ) ) {
		...
		var base = jQuery( '#con_player_ranking_table, #con_ally_ranking_table' );
	...
	}).eq( -1 ).after( '<tr><td colspan="8"><input type="button" id="twa-mapgenerator" value="{generate}"/> <label><input type="checkbox" id="checkall"/> <strong>{selectall}</strong></label></td></tr>'.lang( 'mapgenerator' ) );
